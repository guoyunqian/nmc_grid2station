...
class FixParamTypes(Enum):
    ...
    DDictHDF = 'd_dict_hdf'            #保存hdf格式的目录，一般用于同时生成多个格式文件
    DFnFmtHDF = 'd_fn_fmt_hdf'         #保存hdf格式的目标文件文件名的格式
    SaveHDF = 'save_hdf'               #是否保存hdf
    ...
    DDTFmt = 'd_dt_fmt'                #目标文件名称中时间的格式
    DDTFmts = 'd_dt_fmts'              #多个目标文件名称中时间的格式
    ...
    SrcCols = 'src_cols'               #源数据列名称列表
    DstCols = 'dst_cols'               #目标数据列名称列表
    ...
    InterpType = 'interp_type'         #插值算法类型
    
    ProductDesc = 'product_desc'       #产品描述
    ProductCode = 'product_code'       #产品代码
    CCCC = 'CCCC'                      #CCCC发报中心
