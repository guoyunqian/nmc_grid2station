...
class ZCZCParser():
    ...
    def __parse_sta_infos(self, fdata):
        try:
            ...
            staid = public.convert_sta_id_to_int(splitrst[0])
    ...
    def __parse_sta_data(self, fdata, rowcount, columncount):
        try:
            rst = []
            for i in range(rowcount):
                dl = fdata.readline()
                if dl is None or len(dl) == 0:
                    LogLib.logger.error('ZCZCParser __parse_sta_data format error %s' % (fdata))
                    return None

                colstrs = dl.split()
                if len(colstrs) != columncount:
                    LogLib.logger.error('ZCZCParser __parse_sta_data format error %s' % (fdata))
                    return None

                p = [float(x) for x in colstrs]
                p[0] = int(colstrs[0])
                rst.append(p)

            return rst
        except Exception as data:
            LogLib.logger.error('ZCZCParser __parse_sta_infos except %s %s' % (str(data), fdata))
    ...
    def __parse_data(self, fdata, staids=None, columns=None, filters=None):
        try:
            ...
            for i in range(self.sta_count):
                ...
                staid, lon, lat, alt, rowcount, columncount = self.__parse_sta_infos(fdata)
                if staid is None or lon is None or lat is None or alt is None or rowcount is None or columncount is None:
                    return False
                ...
                if staids is not None and staid not in staids:
                    for j in range(rowcount):
                        dl = fdata.readline().strip()
                        if dl is None or dl == '':
                            LogLib.logger.error('ZCZCParser __parse_data readlines error %s' % (fdata))
                            return False
                ...
                tmpdata = self.__parse_sta_data(fdata, rowcount, columncount+1)
                #print('4  ,',datetime.datetime.now())
                if tmpdata is None:
                    return False

                tmpd = [staid, lon, lat, alt]
                for di in tmpdata:
                    di += tmpd
                ...
                if stadata is None:
                    ...
                    stadata.extend(tmpdata)

                #print('5  ,',datetime.datetime.now())
            
            #print('6  ,',datetime.datetime.now())

            tmpcols = copy.deepcopy(ZczcPublicType.columns)
            tmpcols.extend([ColumnNames.StaID.value, ColumnNames.Lon.value, ColumnNames.Lat.value, ColumnNames.Alt.value])

            pdata = pd.DataFrame(stadata, columns=tmpcols)
